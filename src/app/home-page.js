...
export function renderHome(appData) {
	...
	const { benchmarkIndex, benchmarksData, currentScores, changelog } = appData;

	if (!benchmarkSelect || !questionCountSelect || !preview || !scoreboard || !form) {
		return;
	}

	const changelogEl = document.querySelector('[data-role="changelog"]');
	if (changelogEl && changelog?.summary) {
		const generatedAt = changelog.generatedAt ? formatDate(changelog.generatedAt) : null;
		...
		eyebrow.textContent = `What's new${generatedAt ? ` · ${generatedAt}` : ''}`;
		...
		disclaimer.className = 'eyebrow';
		disclaimer.style.marginTop = '1rem';
		disclaimer.textContent = '⚠ This summary was generated by GitHub Copilot (AI) and may not be fully accurate.';

		article.append(eyebrow, heading, body, disclaimer);
		changelogEl.appendChild(article);
	}
